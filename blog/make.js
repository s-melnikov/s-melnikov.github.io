...
.concat(fs.readFileSync(".ignore", "utf8").split("\r\n"))
.map(str => new RegExp("^\.\/" + str.replace(/\.|\*/g, match => replacer[match] ), "i"))
...
global.render = (name, data) => tpl(fs.readFileSync("layout/" + name + ".html", "utf8"))(data)

console.log("End")
return;

function generate(path, _path = "") {
  console.log("!", path)
  fs.readdirSync(path).map(file => {
    if (fs.statSync(path + "/" + file).isDirectory()) {
      console.log("DIR1:", _path + file)
      fs.mkdirSync(_path + file)
      generate(path + "/" + file, (_path ? _path + "/" : "") + file + "/")
    } else {
      var data = getData(path + "/" + file)
      file = file.slice(0, -3)
      if (file != "index") {
        console.log("DIR2:", _path + file)
        fs.mkdirSync(_path + file)
        file += "/index"
      }
      console.log("FILE:", file + ".html")
      fs.writeFileSync((_path + file + ".html"), "!!!")
    }
  })
}

function getData(path) {
  var rawString = fs.readFileSync(path, "utf8").split("---")
  var content = rawString.slice(2).join("---")
  var excerpt = content.split("\r\n\n")[1]
  var meta = {}
  rawString[1].trim().split("\r\n").map(pair => {
    pair = pair.split(":")
    meta[pair[0].trim().toLowerCase()] = pair.slice(1).join(":").trim()
  })
  console.log(meta)
  console.log(content)
  retrun { meta, content, excerpt }
}

sourceList.map(name => {
  if (/\.md$/i.test(name)) {
    var date = name.slice(0, 10).split("-").reverse().join(".")
    var slug = name.slice(11, -3)
    var content = fs.readFileSync("source/" + name, "utf8")
    var title = /#(.+)\n/.exec(content)[1]
    var excerpt = content.split("\n\n")[1]
    var post = { date, slug, content, title, excerpt }
    posts.push(post)
    fs.mkdirSync("posts/" + slug)
    fs.writeFileSync("posts/" + slug + "/index.html", render("post", post))
  }
})
...
fs.writeFileSync("index.html", render("index", { posts }))

function isIgnore(path) {
  return ignore.filter(regexp => regexp.test(path)).length
}
