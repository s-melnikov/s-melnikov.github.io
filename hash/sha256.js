(function () {
  ...
  const createOutputMethod = (outputType) =>
    message => new Sha256(true).update(message)[outputType]()

  const createMethod = () => {
    let method = createOutputMethod('hex')
    method.create = () => new Sha256()
  ...
  const createHmacOutputMethod = (outputType) => (key, message) =>
    new HmacSha256(key, true).update(message)[outputType]()

  var createHmacMethod = () => {
    let method = createHmacOutputMethod('hex')
    method.create = key => new HmacSha256(key)
  ...
  Sha256.prototype.update = function (message) {
    ...
    if (type !== 'string') {
      if (type === 'object') {
        ...
        } else if (true && message.constructor === ArrayBuffer) {
          message = new Uint8Array(message)
        } else if (!Array.isArray(message)) {
          if (!true || !ArrayBuffer.isView(message)) {
            throw new Error(ERROR)
          }
        }
      } else {
  ...
  Sha256.prototype.hex = function () {
    ...
    let h0 = this.h0, h1 = this.h1, h2 = this.h2, h3 = this.h3, h4 = this.h4,
      h5 = this.h5, h6 = this.h6, h7 = this.h7
  ...
  function HmacSha256(key, sharedMemory) {
    ...
    if (type === 'string') {
      ...
      if (type === 'object') {
        if (key === null) {
          throw new Error(ERROR)
        } else if (true && key.constructor === ArrayBuffer) {
          key = new Uint8Array(key)
        } else if (!Array.isArray(key)) {
          if (!true || !ArrayBuffer.isView(key)) {
            throw new Error(ERROR)
          }
        }
      } else {
  ...
  window.sha256.hmac = createHmacMethod()
})()
